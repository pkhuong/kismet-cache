...
impl<T: Entry> Update<T> {
    ...
    pub fn new(entries: impl IntoIterator<Item = T>, capacity: usize) -> Self {
        ...
        if to_evict.len() < must_remove {
            ...
            assert!(num_remaining <= to_move_back.len());

            let survivors = to_move_back.split_off(num_remaining);
            to_evict.extend(to_move_back);

            to_move_back = survivors;
        }
...
mod test {
    ...
    proptest! {
        ...
        #[test]
        fn compare_touch_oracle(mut inputs in vec(any::<TestEntry>(), 0..20usize),
                                capacity in 1..10usize) {
            let result = Update::new(inputs.clone(), capacity);
            ...
            if result.to_evict.is_empty() {
                ...
                let must_touch: Vec<_> = inputs
                    ...
                    .cloned()
                    .collect();
                assert_eq!(&result.to_move_back[0..must_touch.len()], &must_touch);
                ...
                let may_touch: Vec<_> = inputs
                    ...
                    .cloned()
                    .collect();
                assert_eq!(&result.to_move_back, &may_touch[0..result.to_move_back.len()]);
            }
        }
    }
}
